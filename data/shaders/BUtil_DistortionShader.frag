...
// screen
layout(binding = 0) uniform sampler2D u_texScreen;
...
void main() {
    ...
    float arc = (u_statePackage[5].x > -1.0) ? min(inverseLerp(u_statePackage[5].y, u_statePackage[5].x, vb_data.fragUVMask.x / ring), 1.0) : 1.0;
    float ringHardness = u_statePackage[HARDNESS_RING].w * vb_data.fragGlobalHardness, innerHardness = u_statePackage[HARDNESS_INNER].z * vb_data.fragGlobalHardness;
    ...
    if (mask <= 0.0) discard;
    vec4 result = texture(u_texScreen, vb_data.fragUVScreen.xy - (vb_data.fragUVScreen.zw * mask));
    result.w = 1.0;
    o_fragColor = result;
}
