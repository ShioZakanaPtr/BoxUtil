...
void main() {
    bool directCheck = u_statePackage[CURVE_STATE].x <= 1.0;
    ...
    bool useGlobal = u_statePackage[CURVE_STATE].z > 0.0;
    vec2 ids = vec2(ttb_datas[0].teseID, ttb_datas[1].teseID);
    vec2 uv = useGlobal ? (u_statePackage[CURVE_STATE].z * ids) : (vec2(ttb_datas[0].teseDistance, ttb_datas[1].teseDistance) / u_statePackage[CURVE_STATE].y);
    ...
    vec2 fillMix = smoothstep(u_statePackage[CURVE_FILL].zw, vec2(1.0), vec2(1.0 - fillUV, fillUV)) * (1.0 - u_statePackage[CURVE_FILL].xy);
    fillMix = 1.0 - fillMix;
    ...
    tgb_data.geomUV = mix(uv.x, uv.y, factor1) - u_statePackage[CURVE_STATE].w;
