...
public final class BUtil_SharedRenderingPass {
    public static void layerInit(final ViewportAPI viewport, final BaseShaderPacksContext context, boolean inCampaign) {
        ...
        BUtil_GLImpl.refreshCurrFrameState(viewport, context, inCampaign ? BoxEnum.TRUE : BoxEnum.FALSE);
        layeredRes.applyMemoryBarrier();
        BoxThreadSync.Rendering.beginRendering().arriveAndAwaitAdvance();
    }

    public static boolean highestLayer(final ViewportAPI viewport, final BaseShaderPacksContext context, boolean shaderEnable, boolean notMultiPass, boolean inCampaign) {
        final var layeredRes = inCampaign ? BUtil_ResourceStorage.campaignLayered() : BUtil_ResourceStorage.combatLayered();
        ...
        BUtil_GLImpl.processShaderpacksResultPass(viewport, inCampaign, context);

        final boolean in_notMultiPass = notMultiPass && (BoxConfigs.isMultiPassBeauty() || BoxConfigs.isMultiPassColor());
        if (in_notMultiPass) GLWrapper.Operation.glBlendFuncSeparatei(0, GLWrapper.Operation.GL_SRC_ALPHA, GLWrapper.Operation.GL_ONE_MINUS_SRC_ALPHA, GLWrapper.Operation.GL_ZERO, GLWrapper.Operation.GL_ONE);
        ...
        if (in_notMultiPass) GLWrapper.Operation.glEnable(GLWrapper.Operation.GL_BLEND);
    ...
    public static boolean checkSkipOrRemove(boolean shaderEnable, boolean highestLayer, boolean lowestLayer, final EnumMap<LayeredEntityType, List<RenderDataAPI>> meshMap, final Set<LayeredRenderingPlugin> renderingPlugins, int layerLoc, final Runnable onRemove) {
        ...
        if (stageContinue) {
            if (shaderEnable && highestLayer) ShaderCore.glEndDraw();
    ...
    public static void drawEachLayer(boolean shaderEnable, boolean highestLayer, int layerBits, final Object layer, int layerLoc, boolean canRendering, final ViewportAPI viewport, final EnumMap<LayeredEntityType, List<RenderDataAPI>> meshMap, final Set<LayeredRenderingPlugin> renderingPlugins) {
        ...
        BUtil_EntityImpl.Mesh.processMeshCurrentLayout(layerBits, layer, layerLoc, canRendering, viewport, meshMap, renderingPlugins);
        BUtil_GLImpl.resetGLAttrib();
        if (shaderEnable) {
            if (!highestLayer) {
                GLWrapper.FBO.glBindFramebuffer(GLWrapper.FBO.GL_FRAMEBUFFER, 0);
