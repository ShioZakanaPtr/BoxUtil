...
public class Int2IntRHMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable {
     ...
     */
    public Int2IntRHMap(int initialCapacity, float loadFactor, final ContainerHasher hasher) {
     ...
     * @param hasher when a key is <code>null</code>, it hash must be <code>0</code>
     */
    public Int2IntRHMap(int initialCapacity, final ContainerHasher hasher) {
     ...
     * @param initialCapacity will auto conversion to <code>2^n</code> that greater than or equal to the value.
     */
    public Int2IntRHMap(int initialCapacity) {
    ...
    protected Object clone() {
        Int2IntRHMap<K, V> result;
        try {
            result = (Int2IntRHMap<K, V>) super.clone();
    ...
    protected class Slot implements Entry<K, V> {
        ...
        public final V setValue(V newValue) {
            ...
            Int2IntRHMap.this.kvs[this.realPos] = newValue;
            this.value = newValue;
            return oldValue;
        }
    ...
    protected abstract class RHIterator<T> implements Iterator<T> {
        ...
        protected RHIterator() {
            this.currOp = Int2IntRHMap.this.op;
            this.remaining = Int2IntRHMap.this.capacity;
            if (Int2IntRHMap.this.size > 0) {
                for (int i = 0; i < this.remaining; i++) {
                    if (Int2IntRHMap.this.state[i] > -1) {
        ...
        protected int nextNode() {
            if (Int2IntRHMap.this.op != this.currOp) throw new ConcurrentModificationException();
            ...
            if (this.pos > -1) {
                for (int i = this.pos + 1; i < Int2IntRHMap.this.capacity; i++) {
                    if (Int2IntRHMap.this.state[i] > -1) {
                        this.posNext = i;
                        break;
                    }
        ...
        public void remove() {
            ...
            if (this.posPre < 0 || Int2IntRHMap.this.state[currRealPos] < 0) throw new IllegalStateException();
            if (Int2IntRHMap.this.op != this.currOp) throw new ConcurrentModificationException();
            final int currPos = this.posPre;
            Int2IntRHMap.this.size--;
            Int2IntRHMap.this.state[this.posPre] = BoxEnum.NEG_ONE;
            Int2IntRHMap.this.kvs[currRealPos] = null;
            Int2IntRHMap.this.kvs[currRealPos + 1] = null;

            boolean refreshNextItPos = true;
            for (int i = this.posPre + 1 & Int2IntRHMap.this.posMask; i != currPos; i = ++i & Int2IntRHMap.this.posMask) {
                final int realPos = i << 1;
                if (Int2IntRHMap.this.state[i] < 1) break;

                final int realValPos = realPos + 1, prePos = (i - 1) & Int2IntRHMap.this.posMask, preRealPos = prePos << 1;
                Int2IntRHMap.this.state[prePos] = --Int2IntRHMap.this.state[i];
                Int2IntRHMap.this.kvs[preRealPos] = Int2IntRHMap.this.kvs[realPos];
                Int2IntRHMap.this.kvs[preRealPos + 1] = Int2IntRHMap.this.kvs[realValPos];
                Int2IntRHMap.this.state[i] = BoxEnum.NEG_ONE;
                Int2IntRHMap.this.kvs[realPos] = null;
                Int2IntRHMap.this.kvs[realValPos] = null;
                this.remaining--;
            ...
            Int2IntRHMap.this.calculateThreshold();
            this.currOp = ++Int2IntRHMap.this.op;
        }
    }
    ...
    protected class KeyIterator extends RHIterator<K> {
        public K next() {
            return (K) Int2IntRHMap.this.kvs[this.nextNode()];
        }
    }

    protected class ValueIterator extends RHIterator<V> {
        public V next() {
            return (V) Int2IntRHMap.this.kvs[this.nextNode() + 1];
        }
    }

    protected class EntryIterator extends RHIterator<Entry<K,V>> {
        public Entry<K,V> next() {
            final int realPos = this.nextNode();
            return new Slot(realPos, (K) Int2IntRHMap.this.kvs[realPos], (V) Int2IntRHMap.this.kvs[realPos + 1]);
        }
    }
    ...
    protected abstract class RHSpliterator<T> implements Spliterator<T> {
        ...
        protected final int getLimitCap() {
            ...
            if ((newLimit = this.limitCap) < 0) {
                this.estSize = Int2IntRHMap.this.size;
                currOp = Int2IntRHMap.this.op;
                newLimit = this.limitCap = Int2IntRHMap.this.capacity;
            }
            return newLimit;
        }
        ...
        public int characteristics() {
            return (this.limitCap < 0 || this.estSize == Int2IntRHMap.this.size ? Spliterator.SIZED : 0) |
    ...
    protected class KeySpliterator extends RHSpliterator<K> {
        ...
        public boolean tryAdvance(Consumer<? super K> action) {
            ...
            while (this.index < currLimit) {
                ...
                if (Int2IntRHMap.this.state[currPos] > -1) {
                    action.accept((K) Int2IntRHMap.this.kvs[currPos << 1]);
                    if (Int2IntRHMap.this.op != this.currOp) throw new ConcurrentModificationException();
                    return true;
                }
            }
        ...
        public void forEachRemaining(Consumer<? super K> action) {
            ...
            while (this.index < currLimit) {
                final int currPos = this.index++;
                if (Int2IntRHMap.this.state[currPos] > -1) action.accept((K) Int2IntRHMap.this.kvs[currPos << 1]);
                if (Int2IntRHMap.this.op != this.currOp) throw new ConcurrentModificationException();
            }
        }
    }
    ...
    protected class ValueSpliterator extends RHSpliterator<V> {
        ...
        public boolean tryAdvance(Consumer<? super V> action) {
            ...
            while (this.index < currLimit) {
                final int currPos = this.index++;
                if (Int2IntRHMap.this.state[currPos] > -1) {
                    action.accept((V) Int2IntRHMap.this.kvs[(currPos << 1) + 1]);
                    if (Int2IntRHMap.this.op != currOp) throw new ConcurrentModificationException();
                    return true;
                }
            }
            return false;
        ...
        public void forEachRemaining(Consumer<? super V> action) {
            ...
            while (this.index < currLimit) {
                final int currPos = this.index++;
                if (Int2IntRHMap.this.state[currPos] > -1) action.accept((V) Int2IntRHMap.this.kvs[(currPos << 1) + 1]);
                if (Int2IntRHMap.this.op != this.currOp) throw new ConcurrentModificationException();
            }
        }
    ...
    protected class EntrySpliterator extends RHSpliterator<Entry<K,V>> {
        ...
        public boolean tryAdvance(Consumer<? super Entry<K,V>> action) {
            ...
            while (this.index < currLimit) {
                ...
                if (Int2IntRHMap.this.state[currPos] > -1) {
                    ...
                    action.accept(new Slot(realPos, (K) Int2IntRHMap.this.kvs[realPos], (V) Int2IntRHMap.this.kvs[realPos + 1]));
                    if (Int2IntRHMap.this.op != currOp) throw new ConcurrentModificationException();
                    return true;
                }
            }
        ...
        public void forEachRemaining(Consumer<? super Entry<K,V>> action) {
            ...
            while (this.index < currLimit) {
                ...
                if (Int2IntRHMap.this.state[currPos] > -1) {
                    final int realPos = currPos << 1;
                    tmpStruct.reset(realPos, (K) Int2IntRHMap.this.kvs[realPos], (V) Int2IntRHMap.this.kvs[realPos + 1]);
    ...
    protected class KeySet extends AbstractSet<K> {
        public int size() {
            return Int2IntRHMap.this.size();
        }

        public void clear() {
            Int2IntRHMap.this.clear();
        }
        ...
        public boolean contains(Object o) {
            return Int2IntRHMap.this.containsKey(o);
        }

        public boolean remove(Object key) {
            return Int2IntRHMap.this.getValBool(key, (l_si, l_ki, l_vi) -> {
                Int2IntRHMap.this.removeAndShift(l_si, l_ki, l_vi);
                Int2IntRHMap.this.calculateThreshold();
                return true;
            });
        }
        ...
        public Object[] toArray() {
            return slotToArray(new Object[Int2IntRHMap.this.size], (byte) 0);
        }
        ...
        public void forEach(Consumer<? super K> action) {
            Int2IntRHMap.this.forEach((l_k, l_v) -> action.accept(l_k));
        }
    }
    ...
    protected class Values extends AbstractCollection<V> {
        ...
        public boolean contains(Object o) {
            return Int2IntRHMap.this.containsValue(o);
        }
        ...
        public Object[] toArray() {
            return slotToArray(new Object[Int2IntRHMap.this.size], (byte) 1);
        }
        ...
        public void forEach(Consumer<? super V> action) {
            Int2IntRHMap.this.forEach((l_k, l_v) -> action.accept(l_v));
        }
    }
    ...
    protected class EntrySet extends AbstractSet<Entry<K, V>> {
        ...
        public boolean contains(Object o) {
            if (o instanceof Entry<?, ?> e) return Objects.equals(e.getValue(), Int2IntRHMap.this.get(e.getKey()));;
            return false;
        }

        public boolean remove(Object o) {
            if (o instanceof Entry<?, ?> e) return Int2IntRHMap.this.remove(e.getKey(), e.getValue());;
            return false;
        }
        ...
        public void forEach(Consumer<? super Entry<K,V>> action) {
            Objects.requireNonNull(action);
            if (Int2IntRHMap.this.size < 1) return;
            final Slot tmpStruct = new Slot(0, null, null);
            for (int i = 0; i < Int2IntRHMap.this.capacity; i++) {
                final byte currOp = Int2IntRHMap.this.op;
                final int currPos = i << 1;
                if (Int2IntRHMap.this.state[i] > -1) {
                    tmpStruct.reset(currPos, (K) Int2IntRHMap.this.kvs[currPos], (V) Int2IntRHMap.this.kvs[currPos + 1]);
                    action.accept(tmpStruct);
                }
                if (Int2IntRHMap.this.op != currOp) throw new ConcurrentModificationException();
            }
