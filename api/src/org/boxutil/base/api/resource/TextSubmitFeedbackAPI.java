...
public interface TextSubmitFeedbackAPI {
     ...
     */
    default void processText(byte fontData, int textDataIndex, char character, boolean valid, boolean linefeed, int indexOfCurrString, float cursorX, float cursorY, float dataPadding, byte styleBits, byte colorRed, byte colorGreen, byte colorBlue, byte colorAlpha) {}

    /**
     * When not-any space that draw current character.
     */
    default void processBreak(byte fontData, int textDataIndex, boolean beforeNextSubmit, char character, int indexOfCurrString, float cursorX, float cursorY) {}
}
